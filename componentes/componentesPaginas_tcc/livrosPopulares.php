<?php
include('../BuscaLivros/buscaLivrosPopulares.php');
?>

<?php if (count($livros_populares) > 0): ?>
<?php foreach ($livros_populares as $livro_popular): ?>
...
<?php
        ...
        $stmt->execute([
            ...
            ':livro_cod' => $livro_popular['livro_cod']
        ]);
        $favorito = $stmt->fetchColumn() ? true : false;
        ?>

<div class="livro">
    <img src="<?= htmlspecialchars($livro_popular['livro_capa_link']) ?>" class="imgLivro">
    <div class="gradiente"></div>
    <button class="marcador" data-livro-cod="<?= $livro_popular['livro_cod'] ?>">
        <img src="<?= $favorito ? '../img/bookmark_preenchido.png' : '../img/salvar_livro.png' ?>" class="imgMarcador">
    </button>
    <h6 class="nomeLivro">
        <?= htmlspecialchars($livro_popular['livro_titulo']) ?>
    </h6>
    <h6 class="nomeAutor">
        <?= htmlspecialchars($livro_popular['autor_nome']) ?>
    </h6>
    <?php
    $sql = "SELECT AVG(nota) AS media, COUNT(*) AS total_avaliacoes FROM Avaliacoes WHERE livro_cod = :livro_cod";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':livro_cod', $livro_popular['livro_cod'], PDO::PARAM_INT);
    $stmt->execute();
    ...
    <form name="form_pgLivro" action="pgLivro.php" method="get">
        <input type="hidden" name="livro_cod" value="<?= htmlspecialchars($livro_popular['livro_cod']) ?>">        
